...
const battleUI = {
    ...
    async init() {
        ...
        if (!this.ui.fightButton || !this.ui.runButton || !this.ui.battleLogContainer ||
            ...
            if(this.ui.fightButton) this.ui.fightButton.disabled = true;
            if(this.ui.runButton) this.ui.runButton.disabled = true;
            return;
        }

        this.ui.fightButton.addEventListener('click', () => this.handlePlayerAction('fight_action'));
    ...
    async fetchInitialBattleData() {
        ...
        this.ui.fightButton.disabled = true; 
        ...
        try {
            ...
            if (result.success && result.player_pokemon && result.enemy_pokemon) {
                ...
                if (this.playerPokemon.health > 0 && this.enemyPokemon.health > 0) {
                    this.ui.fightButton.disabled = false;
    ...
    handleBattleOverUI(finalMessage) {
        ...
        this.ui.fightButton.style.display = 'none'; // Ẩn nút Fight
    ...
    async handlePlayerAction(battleApiAction) { 
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.fight_log && Array.isArray(result.fight_log)) {
                    result.fight_log.forEach((round, index) => {
                        this.addLogMessage(`Vòng ${index + 1}: Bạn (HP: ${round.player_hp}, DMG: ${round.player_dmg}) vs Địch (HP: ${round.enemy_hp}, DMG: ${round.enemy_dmg})`);
                    });
                ...
                this.updateBattleMessage(result.outcome_message || result.message || "Lượt tiếp theo...");

                if (result.battle_over) {
                    // Gọi hàm xử lý UI khi trận đấu kết thúc
                    this.handleBattleOverUI(result.outcome_message || "Trận đấu đã kết thúc!");
                    // Không còn setTimeout và chuyển hướng tự động ở đây
                } else {
                    // Nếu trận đấu chưa kết thúc (ví dụ: bỏ chạy thất bại nhưng vẫn còn HP)
                    this.ui.fightButton.disabled = false;
                    this.ui.runButton.disabled = false;
                }
